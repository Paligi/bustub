...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper"){
    ...
    if (func_name == "lower"){
      return std::make_shared<StringExpression>(args.back(), StringExpressionType::Lower);
    }
    if (func_name == "upper"){
      return std::make_shared<StringExpression>(args.back(), StringExpressionType::Upper);
    }
